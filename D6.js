...
function stringhe(stringa1, stringa2) {
  ...
  const stringaConcatenata = primaStringa + secondaStringa;
  const stringaMaiuscola = stringaConcatenata.toUpperCase();

  console.log(stringaMaiuscola);
}

const stringa1 = "pippo";
const stringa2 = "baudo";
...
const arrayDiNumeriRandom = array();
...
const arrayDiNumeriPari = pari(numeri);
...
const somma = sommaNumeri(arrayDiNumeri);
console.log(somma);
/* ESERCIZIO 5 (reduce)
  Scrivi una funzione per sommare i numeri contenuti in un array
*/
...
*/
function valori(arrayNumeri, n) {
  const arrayIncrementato = arrayNumeri.map((numero) => numero + n);
  return arrayIncrementato;
}
const arr = [1, 2, 3, 4, 5];
const n = 3;
const arrayIncrementato = valori(arr, n);
...
const arrayDiLunghezze = lunghezzeStringhe(arrayDiStringhe);
console.log(arrayDiLunghezze);
...
const arrayDiNumeriDispari = numeriDispari();
